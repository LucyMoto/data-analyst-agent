...
def make_data_analyst_agent(model, checkpointer: Optional[object] = None):
    ...
    def run_eda_node(state: OrchestrationState) -> dict:
        ...
        data_to_analyse = state.get("data_cleaned")
        if data_to_analyse is None:
            logger.warning("No cleaned data available for EDA")
            return {"eda_response" : {}}
        
        # Invoke eda_graph with the cleaned data and return the response.
    ...
    # Assemble the graph — add nodes, set entry point, and wire edges.
